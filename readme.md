...
├── .gitignore                   # Git ignore file for excluding local files (.venv, .cursor, cache)
...
├── llc4320_latlon.nc           # NetCDF file containing latitude/longitude data for the dataset
...
- **`.gitignore`**: Git ignore file that excludes local development files from version control, including `.venv` (virtual environment), `.cursor` (editor files), and `.visus_cache_can_be_deleted` (OpenVisus cache directory).
...
- **`LLC4320_metadata.ipynb`**: A secondary demo notebook providing additional examples of how to work with the LLC4320 dataset, including metadata extraction and other data manipulation techniques.

- **`llc4320_latlon.nc`**: A NetCDF file containing latitude and longitude coordinate information for the LLC4320 dataset, used in the metadata notebook examples.
